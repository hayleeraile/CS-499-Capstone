...
//allows a user to create a new inventory item. validates the input and records who added the item
public class AddItemActivity extends AppCompatActivity {
    ...
    //validates the entered item data before it gets saved
    private void saveItem() {
        ...
        if (id != -1) {
            //records which logged in user created the item
            SharedPreferences prefs = getSharedPreferences("app_prefs", MODE_PRIVATE);
            long userId = prefs.getLong("current_user_id", -1);

            if (userId != -1) {
                dbHelper.addItemHistory(id, userId, "ITEM_ADDED");
            }
