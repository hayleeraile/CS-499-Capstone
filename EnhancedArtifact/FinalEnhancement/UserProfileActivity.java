...
//displays the logged in users profile, allows updates and provides an option to logout.
public class UserProfileActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //retrieves the current username so the correct profile can be updated
        SharedPreferences prefs = getSharedPreferences("app_prefs", MODE_PRIVATE);
        currentUsername = prefs.getString("current_username", "");

        //prevents access to this screen if there is no one logged in
    ...
    //loads currently authenticated users profile from the database
    ...
    //validates any changes and saves them to the database
    ...
    private void logoutUser() {
        SharedPreferences prefs = getSharedPreferences("app_prefs", MODE_PRIVATE);
        prefs.edit().remove("current_username").remove("current_user_id").apply();

        Toast.makeText(this, getString(R.string.logout_success), Toast.LENGTH_SHORT).show();

        Intent intent = new Intent(this, MainActivity.class);

        //clears the activity so the back button cannot reopen the screen after logout.
